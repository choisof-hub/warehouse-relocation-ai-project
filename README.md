...
【参考】
以前の倉庫移転では棚番変換用の新マスターを全て手作業で作成したため、完成までに約2か月を要しました。

## 概要

倉庫移転に伴い、既存の棚番体系を全面的に見直し、新しい棚番へ移行する必要がありました。
変更自体はシステム部門が用意した一括更新プログラムを使用しますが、そのプログラムに読み込ませる、新し棚番マスターを作成する必要性がありました。
...
を作成し、移転作業から移転直後の出荷業務までを円滑に実施できる、棚番マスターの作成環境を構築しました。
...
旧棚番配置図と新棚番配置図を基に、新旧棚番変換表を自動生成するツールです。これを棚番の一括更新プログラムに読み込ませます。
